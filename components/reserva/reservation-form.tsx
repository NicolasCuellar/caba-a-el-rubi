...
const cabinOptions = [
  { value: "cabana-a", label: "Cabaña Rubi", maxPersonas: 4 },
  { value: "cabana-b", label: "Cabaña Zafiro", maxPersonas: 4 },
  { value: "cabana-c", label: "Cabaña Esmeralda", maxPersonas: 8 },
]
...
function esMismoDiaYPasaDe3PM(fecha: string): boolean {
  if (!fecha) return false
  const hoyCol = new Date().toLocaleDateString("en-CA", { timeZone: "America/Bogota" })
  if (fecha !== hoyCol) return false
  // Hora actual en Colombia
  const horaCol = new Date().toLocaleTimeString("en-US", {
    timeZone: "America/Bogota",
    hour12: false,
    hour: "2-digit",
    minute: "2-digit",
  })
  const [h, m] = horaCol.split(":").map(Number)
...
export function ReservationForm() {
  ...
  const checkInTardio = useMemo(() => esMismoDiaYPasaDe3PM(fechaEntrada), [fechaEntrada])
  ...
  }, [cabana, fechaEntrada, fechaSalida, checkInTardio])
        ...
        <div className="rounded-2xl bg-card p-8 shadow-2xl md:p-10">
          ...
          <form onSubmit={handleMostrarResumen} className="flex flex-col gap-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input id="fechaSalida" type="date" required
                  min={
                    fechaEntrada
                      ? new Date(new Date(`${fechaEntrada}T12:00:00`).getTime() + 86400000)
            ...
            {checkInTardio && (
              <div className="flex items-center gap-2 rounded-lg border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm text-destructive">
                <AlertCircle className="h-4 w-4 shrink-0" />
                El check-in es a las 3:00 p.m. Ya pasó ese horario para hoy. Por favor selecciona otra fecha.
              </div>
            )}
            ...
            <Button type="submit" size="lg"
              disabled={noDisponible || verificando || checkInTardio}

...
import {
  ...
  notificarClientePasadia,
  notificarAdminNuevaReserva,
} from "@/lib/whatsapp"
import {
  notificarClienteReservaEmail,
  notificarAdminReservaEmail,
...
function construirDescripcion(datos: DatosReserva, estado: EstadoReserva): string {
  ...
  return [
    ...
    ``,
    `── ✏️  Cómo cambiar el estado (edita el TÍTULO del evento) ──`,
    `Copia y pega el título que corresponda:`,
    ``,
    `✅ CONFIRMADA — ${datos.nombre} — ${CABANA_LABEL[datos.cabana]}`,
    `💙 SALDO PENDIENTE — ${datos.nombre} — ${CABANA_LABEL[datos.cabana]}`,
    `❌ CANCELADA — ${datos.nombre} — ${CABANA_LABEL[datos.cabana]}`,
    `🔒 BLOQUEADO — ${CABANA_LABEL[datos.cabana]} — Mantenimiento`,
    ``,
    `── Observaciones ──`,
...
function construirDescripcionPasadia(datos: DatosPasadia, estado: EstadoReserva): string {
  ...
  return [
    ...
    ``,
    `── ✏️  Cómo cambiar el estado (edita el TÍTULO del evento) ──`,
    `Copia y pega el título que corresponda (una línea por cada cabaña):`,
    ``,
    ...datos.cabanas.map((c) =>
      `✅ CONFIRMADA — Pasadía — ${datos.nombre} (${datos.personasPorCabana[c]} pax) — ${CABANA_LABEL[c]}`
    ),
    ``,
    ...datos.cabanas.map((c) =>
      `💙 SALDO PENDIENTE — Pasadía — ${datos.nombre} (${datos.personasPorCabana[c]} pax) — ${CABANA_LABEL[c]}`
    ),
    ``,
    ...datos.cabanas.map((c) =>
      `❌ CANCELADA — Pasadía — ${datos.nombre} (${datos.personasPorCabana[c]} pax) — ${CABANA_LABEL[c]}`
    ),
    ``,
    `── Observaciones ──`,
    datos.observaciones || "(sin observaciones)",
...
export async function crearSolicitudReserva(datos: DatosReserva): Promise<string> {
  ...
  // RF-16: Notificaciones automáticas (best-effort, no bloquean el flujo)
  await Promise.allSettled([
    ...
    notificarAdminNuevaReserva({
      ...
      cabanas:         [datos.cabana],
      fechaEntrada:    datos.fechaEntrada,
      fechaSalida:     datos.fechaSalida,
      personas:        datos.personas,
    }),
    notificarClienteReservaEmail(datos),
    notificarAdminReservaEmail(datos),
  ])
...
// RF-16/RF-17: Notificaciones automáticas (best-effort, no bloquean)
await Promise.allSettled([
  notificarClientePasadia({
    telefono:      datos.telefono,
    nombre:        datos.nombre,
    fecha:         datos.fecha,
    cabanas:       datos.cabanas,
